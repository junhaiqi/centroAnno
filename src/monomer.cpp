...
void monoTempInference(SEQ &sequence, const int &k, const float &kIdenCf,
  ...
  if (!rep){
    std::cout << "[" << currentTimestamp() << "] [centroAnno] " << sequence.seqName
              << " subsequence (length: " << sequence.seq.size()
              << " bp) is not repetitive (low non-unique k-mers)." << std::endl;
    return;
  }
  kCounter.samplingKmers(kIdenCf, kCountCf);
  if (kCounter.selectedKmers.empty()) {
    std::cout << "[" << currentTimestamp() << "] [centroAnno] This sequence may not contain a repeat unit." << std::endl;
    return;
  }
