...
#pragma omp parallel for num_threads(                                          \
  ...
  std::cout << "[" << currentTimestamp() << "] [centroAnno] Clustering complete: " << consensuses.size()
            << " template(s) inferred, " << (idx - consensuses.size())
            << " template(s) filtered." << std::endl;
}
...
void refineDemcompose(const SEQ &longSequence, vector<string> &newMonoTems,
  ...
  std::cout << "[" << currentTimestamp() << "] [centroAnno] Bad regions found: " << badRegions.size() << "\n";
  ...
  if (!badRegions.empty()) {
    for (size_t t = 0; t < 1;
      ...
      std::cout << "[" << currentTimestamp() << "] [centroAnno] Parsing bad regions ...\n";
      while (!badRegions.empty()) {
        ...
        if (!monosForBadRegion.empty()) {
          ...
          vector<int> clusterLabels;
          std::cout << "[" << currentTimestamp() << "] [centroAnno] Clustering refined monomer templates ...\n";
          sampleClustering(segForMonosInfer, tempDemcompBlockRes, clusterLabels,
                           refinedMonoTemplates, threadNum);
          std::cout << "[" << currentTimestamp() << "] [centroAnno] Refined templates generated, redecomposing ...\n";
  ...
  if (badSignal) {
    std::cout << "[" << currentTimestamp() << "] [centroAnno] Too many bad blocks found, reinferring monomer templates and "
              << "redecomposing ...\n";
