...
void inferHORs(
  ...
  std::cout << "[" << currentTimestamp() << "] [centroAnno] Most frequent HOR length: " << majorHORLen
            << " monomer(s)\n";
  ...
  if (HORs.empty()) {
    std::cout << "[" << currentTimestamp() << "] [centroAnno] HOR feature not detected for "
              << monomerDecompBlockRes[0].read_name << ".\n";
    return;
  }
