...
/**
 * Prepare input file path. If path is "-", read stdin into a temp file
 * and return the temp path. Otherwise return the original path.
 */
static std::string prepareInputFile(const char *path, bool &isTemp) {
  isTemp = false;
  if (strcmp(path, "-") != 0) {
    return std::string(path);
  }

  char tmpFile[] = "/tmp/centroAnno_XXXXXX";
...
static void cleanupTempFile(const std::string &path) {
  if (!path.empty() && path.substr(0, 4) == "/tmp") {
    unlink(path.c_str());
  }
}
...
static void depSeq2Monoblock(const char *fn, int &windowSize,
  ...
  while (kseq_read(ks) >= 0) {
    std::string seq = ks->seq.s;
    std::string name = ks->name.s;
    std::cout << "[" << currentTimestamp() << "] [centroAnno] Sequence: " << name << " (" << seq.length() << " bp)\n";
    if (seq.length() < lengthCutoff) {
      std::cout << "[" << currentTimestamp() << "] [centroAnno] Skipping " << name << ": length < " << lengthCutoff
                << " bp (unsuitable for repeat annotation)." << std::endl;
      continue;
    }
    windowSize = seq.length() > windowSize ? windowSize : seq.length();
    ...
    if (monoTemFa == "") {
      monoTempInference(mySeq, k, fpsCutoff, 4, monoSeqs, bestMonos, hpc,
                        repCutoff);
      if (bestMonos.empty()) {
        std::cout << "[" << currentTimestamp() << "] [centroAnno] " << name
                  << " may not be a repeating sequence (empty inferred templates).\n";
        continue;
      }
    }
    ...
    // HOR detection is now handled by the external detectHORs.py script.
...
void annoGenome(const char *fn, int &windowSize, const float &repCutoff,
                ...
                const bool onlyScan = false, const int maxRegionLength = 1000000, const int minRegionLength = 100, const float genomeCutoff = 0.8) {
  gzFile fp;
  kseq_t *ks;
  if ((fp = gzopen(fn, "r")) == 0)
    return;
  ...
  while (kseq_read(ks) >= 0) {
    ...
    std::cout << "[" << currentTimestamp() << "] [centroAnno] Scanning tandem repeat regions ...\n";
    repeatRegionInference(sequence, k, repCutoff, lengthCutoff, hpc,
                          repRegions, maxRegionLength);
    std::cout << "[" << currentTimestamp() << "] [centroAnno] Found " << repRegions.size() << " repeat region(s).\n";
    std::cout << "\n";
    ...
    for (size_t i = 0; i < repRegions.size(); ++i) {
      ...
      std::cout << "--------------------------------------------------------------------------------\n";
      std::cout << "[Region " << (i + 1) << "/" << repRegions.size() << "] "
                << "Position: " << repRegions[i].first << " - " << repRegions[i].second
                << " | Length: " << thisSeq.length() << " bp\n";
      std::cout << "--------------------------------------------------------------------------------\n";
      ...
      std::cout << "--------------------------------------------------------------------------------\n";
      std::cout << "[Region " << (i + 1) << "/" << repRegions.size() << "] Done\n";
      std::cout << "--------------------------------------------------------------------------------\n";
      std::cout << "\n";
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (outDir.empty() || argc - o.ind < 1) {
    ...
    fprintf(stderr, "  -m STR     Specify the monomer template file with fasta "
                    "type [default = None]\n");
    fprintf(stderr, "  -k INT     Specify the k-mer size [default = 10]\n");
    fprintf(stderr, "  -f FLOAT   Specify the fps cutoff [default = 0.6]\n");
    fprintf(stderr,
            "  -r FLOAT   Specify the repeat redio cutoff [default = 0.3]\n");
    ...
    fprintf(stderr, "  example command: cat test.fa | %s -o test -\n", argv[0]);
    return 1;
  }

  bool isTemp = false;
  std::string inputFile = prepareInputFile(argv[o.ind], isTemp);
  if (inputFile.empty()) {
    return 1;
  }
  const char *inputPath = inputFile.c_str();

  if (annoMode == "anno-sat-asm")
  {
    createDirectory(outDir);
    depSeq2Monoblock(inputPath, windowSize, repCutoff, hpc, k, threadNum,
  ...
  {
    createDirectory(outDir);
    annoGenome(inputPath, windowSize, repCutoff, hpc, k, threadNum, fpsCutoff,
