...
inline void writeHORDecomposeRes(const std::vector<HORINFO> &horDecomoseRes,
  ...
  outFile << "Sequence name,HOR name,Strand,Start position,End position,Estimated "
             "Identity,Length,Confidence"
          << "\n";
  for (const auto &a : horDecomoseRes) {
    const size_t len = a.end_pos - a.start_pos + 1;
    std::string hor_name;
    hor_name.reserve(a.monomer_name.size());
    std::string strand = "+";
    for (char c : a.monomer_name) {
      if (c == '\'') {
        strand = "-";
      } else {
        hor_name += c;
    ...
    std::string s =
        a.read_name + "," + hor_name + "," + strand + "," + std::to_string(a.start_pos) +
...
inline void writeHORDecomposeResForGenome(const std::vector<HORINFO> &horDecomoseRes,
  ...
  for (const auto &a : horDecomoseRes) {
    ...
    std::string s =
        a.read_name + "," + "Pos:" + std::to_string(startPos) + ":" + std::to_string(endPos) + ":" + hor_name + "," + strand + "," + std::to_string(a.start_pos + startPos) +
