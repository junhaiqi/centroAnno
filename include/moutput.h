...
inline void writeDecomposeRes(const std::vector<MonomerAlignment> &decomoseRes,
  ...
  outFile << "sequence name,monomer name,strand,start position,end position,estimated "
             "identity,length"
          << "\n";
  for (const auto &a : decomoseRes) {
    const size_t len = a.end_pos - a.start_pos + 1;
    std::string mono_name = a.monomer_name;
    std::string strand = "+";
    if (!mono_name.empty() && mono_name.back() == '\'') {
      mono_name.pop_back();
      strand = "-";
    }
    std::string s = a.read_name + "," + mono_name + "," + strand + "," +
...
inline void writeDecomposeResForGenome(const std::vector<MonomerAlignment> &decomoseRes,
  ...
  for (const auto &a : decomoseRes) {
    ...
    std::string s = a.read_name + "," + "Pos:" + std::to_string(startPos) + ":" + std::to_string(endPos) + "_" + mono_name + "," + strand + "," +
